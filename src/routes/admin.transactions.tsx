    ...
    <div className="space-y-4 animate-fade-up">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="tile-press flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-gold to-amber-700 px-3 py-2 text-xs font-bold text-gold-foreground"
        >
      ...
      <div className="flex items-center gap-2 overflow-x-auto">
        ...
        {tabs.map((t, i) => (
          <button
            key={t}
            className={`tile-press shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold ${i === 0 ? "bg-primary/20 text-primary" : "glass text-muted-foreground"}`}
          >
            {t}
          </button>
        ))}
