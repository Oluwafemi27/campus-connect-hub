...
function DataPage() {
  ...
  return (
    <>
        ...
        {plans.map((p, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-semibold">{p.label} • ₦{p.price.toLocaleString()}</span>
            </div>
            {p.badge && <span className="rounded-full bg-destructive px-2 py-0.5 text-[9px] font-bold">{p.badge}</span>}
            <span className="rounded-lg bg-primary/20 px-3 py-1 text-xs font-bold text-primary">Buy</span>
          </button>
        ))}
      </div>
      ...
      <button
        onClick={() => { if (picked === null || !phone) { toast.error("Pick a plan & enter phone"); return; } toast.success("Data bundle queued"); }}
